...
class KITTIRAWDataset(KITTIDataset):
    ...
    def get_pose(self, folder, frame_index, offset):
        ...
        imu2velo = read_calib_file(
            os.path.join(
                self.data_path, os.path.dirname(folder), "calib_imu_to_velo.txt"
            )
        )
        velo2cam = read_calib_file(
            os.path.join(
                self.data_path, os.path.dirname(folder), "calib_velo_to_cam.txt"
            )
        )
        cam2cam = read_calib_file(
            os.path.join(
                self.data_path, os.path.dirname(folder), "calib_cam_to_cam.txt"
            )
        )
